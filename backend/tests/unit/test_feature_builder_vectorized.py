...
These verify that future vectorised refactors of `_compute_as_of_features`
produce **bit-for-bit-equivalent** output (within float tolerances) to the
legacy implementation kept in `_compute_horse_stats_legacy` /
`_compute_class_change_legacy` / `_compute_last3f_rank_legacy`.

Today the orchestrator still calls the legacy helpers, so the equivalence
check reduces to "legacy == legacy" (a sanity check on the test
infrastructure itself). Once the vectorised path is wired up in commit 2,
these same tests catch any divergence.
"""
...
from services.feature_builder import _compute_as_of_features  # noqa: E402
...
# ---------------------------------------------------------------------------
# Boundary tests (legacy-only; will be augmented in commit 2)
# ---------------------------------------------------------------------------
...
def test_grade_n_starts_zero_value_present(synth_df: pd.DataFrame) -> None:
    ...
    Future vectorised path must preserve this — `(cum / n).where(n>0)` would
