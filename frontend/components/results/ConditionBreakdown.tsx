    ...
    <div className="bg-surface-raised border border-surface-border rounded-xl p-4 space-y-3">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.map((item, i) => {
              const label = [item.surface, item.track_condition].filter(Boolean).join("・") || item.distance_category || `Row ${i}`;
              ...
              return (
                <tr key={i} className="border-b border-surface-border/50">
                  ...
                  <td className={`text-right py-2 px-2 font-mono ${rowBlurred ? "blur-overlay" : "text-text-secondary"}`}>
                    {item.hit_rate?.toFixed(1)}%
                  </td>
                  <td className={`text-right py-2 px-2 font-mono ${rowBlurred ? "blur-overlay" : ""} ${
                    !rowBlurred && item.roi !== null
                      ? item.roi >= 100 ? "text-success" : "text-danger"
                      : "text-text-muted"
                  }`}>
                    {rowBlurred ? "---%" : item.roi !== null ? `${item.roi.toFixed(1)}%` : "---"}
                  </td>
                </tr>
              );
            })}
